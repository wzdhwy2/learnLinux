...
log="log/` echo $id `_` echo $constant $1 $2 `_` date +%Y%m%d `_` date +%T `.log"
#定义日志文件的目录和格式。
...
content_S=` ls *.yml `
#为下面修改ansible-playbook修改执行方式做完善，一修改就修改所有的.yml后缀的文件比较好，就像修改版本号一样。
...
do
		...
		then
			...
			for content_S_S in $content_S
			do
				sed -i 's/#-serial: 1/'"` grep serial: $content_S_S | tr "#-" "  " `"'/g'  $content_S_S
                        	#为了替换 #-serial: 1 ,用sed内容替换，用变量存，用sed整行替换，百度替换空格，最后使用主要tr命令解决
			done
			...
			for content_S_S in $content_S
                        do
                                sed -i 's/'"` grep serial: $content_S_S | tr "  " "  " `"'/#-serial: 1/g' $content_S_S
                        done
...
do
	...
	then
		...
		do
			let wcs--
			$command $name $constant $1 $2 &&  echo "$name $constant $1 $2 执行成功 `date +%T`">>$log || echo "$name $constant $1 $2 执行失败 `date +%T`">>$log 
			sleep 3 | echo "
