...
[ $free -le $freeS ] && echo " 服务器IP:" $ip " ，物理内存共：" $total " M ， 剩余" $free " M ，已不足10%！" |  mail -s  "$ip 服务器 [内存]预警"  wzdhwy2@163.com && date &>>~/free.log
...
[ 0 -le $useS ] && echo "服务器IP：$ip ，根磁盘共：$size ，已使用$used ， 剩余$avail ，使用率将超过$use! " |  mail -s  "$ip 服务器 [磁盘]预警"  wzdhwy2@163.com && date &>>~/df.log
# 比0小 是为了测试，95才是正常
